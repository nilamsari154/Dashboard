...
def clean_dataframe(df, columns):
    ...
    for col in columns:
        if col not in df.columns:
            df[col] = 1 if col == "Quantity" else ""

    return df

    

def send_new_request_notification_to_admin (
        record_id, requestor, requestor_email, category, details, quantity=1,
...
def send_status_change_Requestor_email_to_user(record_id, requestor_email, new_status, old_status=None,
    ...
    <body>
        <div class="email-container">
            ...
            return False, str(e)
print(globals().get("send_status_change_Requestor_email_to_user"))
...
def show_report_month():   
    ...
    st.markdown("")
    report_month_str = st.radio("Select Month", month_names, index=this_month - 3, horizontal=True)
...
def data_system_monitoring_page():
    st.markdown("""
        <div class="dev-hero">
            <div>
                <h1>Data System Monitoring</h1>
                <p>
                    DEV Dashboard is a comprehensive monitoring platform designed
                    to provide real-time insights and tracking of system performance.
                </p>
            </div>
        </div>
    """, unsafe_allow_html=True)
    ...
    for tab, (system, filename) in zip(
        tabs,
        SYSTEMS.items()
    ):
            ...
            try:
                ...
                with open(
                    ...
                    conn.retrieveFile(
                        ...
                        os.path.join(
                            folderName,
                            f"{system}_{report_month_str}_{report_year}.jpg"
                        ),
                        temp
                    )
                ...
                st.warning(
                    f"{system} report for "
                    f"{report_month_str} {report_year} "
                    f"is not available."
                )
                ...
                if "link" in data:
                    ...
                    cols[j].markdown(f'''
                        <a href="{link}" style="text-decoration: none;" target="_blank">
                            <button style="background-color:#0A8276; color:white; border: 1px white solid; border-radius: 8px; padding: 15px 25px; font-size: 22px; display: flex; align-items: center; justify-content: wide; width: 100%; height: 100px; margin-bottom: 10px;">
...
def create_request_page():
    ...
    with st.form("create_request_form", clear_on_submit=False):
        ...
        with col1:
            ...
            requestor = st.text_input("Requestor", disabled=True)
            category = st.selectbox("Category", ["Innovation", "Spare Part Replacement", "YIP/Improvement", "Others"])
            quantity = st.number_input("Quantity", min_value=1)
            color = st.selectbox("Color", ["Black", "White", "Blue", "Grey", "Others"])
            details = st.text_area("Project Details",  placeholder="Enter the project details", height=100)  

        with col2:
            #requestor_email = st.text_input("Requestor Email", value=st.session_state.current_email, disabled=True)
            requestor_email = st.text_input("Requestor Email", disabled=True)
            target_date = st.date_input("Target Date *", value=datetime.now() + timedelta(days=7))    
            material = st.selectbox("Material", ["PLA", "PETG", "ABS", "TPU", "PP", "PC", "PAHT-CF", "Nylon", "Other"])
            has_drawing = st.selectbox("3D Drawing Available?*", ["", "YES", "NO"])
            uploaded_file = st.file_uploader("Upload 3D File Attachment",
...
def my_request_page():
    ...
    st.subheader("📋 My Request")

    st.dataframe(
        {
            "Request No": ["REQ001", "REQ002"],
            "Category": ["Innovation", "YIP"],
            "Status": ["Printing Process", "Completed"]
        },
        width=True
    )
                ...
                with cols[i]:
                    st.markdown(f"""
                        ...
                        display: flex; flex-direction: column; justify-content: center;">
                            <p style="margin:0; color:#64748b; font-size: 1.5rem;">{label}</p>
                            <h3 style="margin:8px 0 0; color:#1e293b; font-size:2.0rem;">{value}</h3>
                        </div>
                    """, unsafe_allow_html=True)
...
def general_information_page():
    ...
    tab1, tab2, tab3= st.tabs(["🖨️ Machine Specs", "📊 Workflow", "🧵 Materials"])
...
# ==== IDENTIFIKASI ROLES ==================
    ...
    print(role_match)
    if st.session_state.current_role != "Admin":
        st.warning("Only admin can access Activity Log")
        return
...
# ====== LOG SUMMARY =========
    ...
    col2.metric("Total Log Activity", len(log_df))
...
def eform_page():
    ...
    if selected_menu == "Create Request":
        create_request_page()

    elif selected_menu == "My Request":
        my_request_page()

    elif selected_menu == "Dashboard":
        dashboard_page()

    elif selected_menu == "General Information":
...
#Sidebar
with st.sidebar:
    st.image("infineon_logo.png", width=True)
