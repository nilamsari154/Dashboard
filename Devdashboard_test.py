...
def create_request_html(record_id, requestor, requestor_email, category, details, quantity=1, material='N/A', color='N/A', target_date=None):
    ...
    safe_details = html.escape(str(details))[:800]
    
    return f"""
    <table width="100%" bgcolor="#f4f7f9" style="font-family: Arial, sans-serif; padding: 20px;">
        <tr>
            <td align="center">
                <table width="600" bgcolor="#ffffff" style="border-radius: 10px; overflow: hidden; border: 1px solid #ddd;">
                    <tr>
                        <td align="center" style="background: #007bff; padding: 30px; color: white;">
                            <h1 style="margin: 0;">New Request #{record_id}</h1>
                        </td>
                    </tr>
                    <tr>
                        <td style="padding: 30px;">
                            <p><strong>Requestor:</strong> {requestor} ({requestor_email})</p>
                            <p><strong>Category:</strong> {category}</p>
                            <p><strong>Quantity:</strong> {quantity} | <strong>Material:</strong> {material} | <strong>Color:</strong> {color}</p>
                            <p><strong>Target Date:</strong> {t_date}</p>
                            <hr style="border: 0; border-top: 1px solid #eee;">
                            <p><strong>Description:</strong></p>
                            <p style="background: #f9f9f9; padding: 15px; border-radius: 5px;">{safe_details}</p>
                        </td>
                    </tr>
                </table>
            </td>
...
def send_email_alt(subject, html_body, to_emails, attachment=None):
    """Universal SMTP Sender (Replaces win32com)"""
    try:
        msg = EmailMessage()
        msg['Subject'] = subject
        msg['To'] = to_emails
...
def ensure_file_exists(file_path):
    if not os.path.exists(file_path):
        ...
        pd.DataFrame(columns=cols).to_excel(file_path, index=False, engine='openpyxl')

@st.cache_data(ttl=2)
def load_Requests():
    ensure_file_exists(REQUESTS_FILE)
    try:
        df = pd.read_excel(REQUESTS_FILE, dtype=str, engine='openpyxl') 
...
def data_system_monitoring_page():
    ...
    # Mapping local files to remote paths
    image_tasks = [
        ('static/devsmets.jpg', f'DEVSPACE_{report_month_str}_{report_year}.jpg'),
        ('static/pv.jpg', f'PV_{report_month_str}_{report_year}.jpg'),
        ('static/nica.jpg', f'NICA_{report_month_str}_{report_year}.jpg')
    ]

    st.markdown("---")
    
    # Verify SMB Connection is active
    if conn is None:
        st.error("📡 Shared drive connection is offline. Please check your credentials or network.")
        return
        ...
        for local_path, remote_filename in image_tasks:
            ...
            with open(local_path, "wb") as temp_file:
                try:
                    ...
                    # Display UI based on which file we are processing
                    if "DEVSPACE" in remote_filename:
                        st.subheader(f"📊 Devspace Monthly Monitoring: {report_month_str} {report_year}")
                        st.image(local_path, use_container_width=True)
                        st.markdown("---")
                    elif "PV_" in remote_filename:
                        st.subheader(f"📈 PV Monthly Monitoring: {report_month_str} {report_year}")
                        st.image(local_path, use_container_width=True)
                        st.markdown("---")
...
def eform_page():
    ...
    def send_outlook_email(to_emails, subject, html_body, attachment=None):
        """SMTP replacement for Outlook sender - No win32com required"""
        try:
            if not to_emails:
            ...
            if not is_valid_email(to_emails):
                return False, f"Invalid email format: {to_emails}"

            # Setup the Email Message
            msg = EmailMessage()
            msg['Subject'] = subject
            msg['To'] = to_emails
            msg['From'] = user  # From your .env config
            msg.add_alternative(html_body, subtype='html')
    ...
    def send_outlook_Requestor_email(to_Requestor_emails, subject, html_body, attach=None):
        """
        Fixed version: Replaces Outlook automation with SMTP.
        Uses credentials from your env_config.
        """
        try:
            ...
            # Note: Ensure normalize_email and is_valid_email are defined in your script
            to_Requestor_emails = normalize_email(to_Requestor_emails)
            if not is_valid_email(to_Requestor_emails):
                return False, "Invalid email address"

            # 2. Construct the Email Message
            msg = EmailMessage()
            msg['Subject'] = subject
            msg['To'] = to_Requestor_emails
            msg['From'] = user  # Reusing your 'user' from env_config
            msg.add_alternative(html_body, subtype='html')
    ...
    def create_enhanced_new_request_html(record_id, requestor, requestor_email, category, details, quantity=1,
        ...
        """Optimized HTML for Email Clients (Outlook/Gmail Compatible)"""
    ...
    def send_new_request_notification_to_admin(record_id, requestor, requestor_email, category, details, quantity=1,
                                               material='N/A', color='N/A', target_date=None, attachment_path=None):
        ...
        try:
            ...
            if not admin_emails:
                admin_emails = [
                    "Rahmad.Hardiansyah@infineon.com, Catur.Pranoto@infineon.com, SitiHanafiNilam.Sari@infineon.com"]
            admin_emails = "Rahmad.Hardiansyah@infineon.com; Catur.Pranoto@infineon.com; SitiHanafiNilam.Sari@infineon.com"
    ...
    user_info = st.session_state.user_df[
        st.session_state.user_df['Requestor_email'].str.lower() == Username.lower()
        ]
    ...
    if not user_info.empty:
        ...
        st.session_state.current_requestor_name = Username.split('@')[0].replace('.', ' ').title()
    ...
    # ======================== MY REQUESTS ===========================
        ...
        if df.empty:
            ...
            my_requests = df[df['Requestor_email'].str.lower() == st.session_state.Username.lower()]
    ...
    # ======================== USER MANAGEMENT ===========================
        ...
        if Username  not in ["sarsitihanaf", "Hardians", "Pranoto"]:
    ...
    # ===================== ACTIVITY LOG =====================
        ...
        # FIXED: Correct string comparison for access control
        allowed_admins = ["sarsitihanaf", "Hardians", "Pranoto"]
        if st.session_state.get('Username') not in allowed_admins:
            st.error("🚨 Access Denied: You do not have permission to view the Activity Log.")
            st.stop()

        st.markdown("<h1 style='text-align:left;'>📊 Activity Log</h1>", unsafe_allow_html=True)
        st.markdown("---")

        @st.cache_data(ttl=60)
        def parse_activity_log(requests_df):
            def extract_events(row):
                history_raw = str(row.get('Status History', '')).strip()
                try:
                    ...
                    for i, event in enumerate(history):
                        ...
                        # FIXED: Removed getpass (Server side) used session state or generic Admin
                        actor = requestor if i == 0 else "Admin"
        ...
        log_df = parse_activity_log(requests_data)
        ...
        st.dataframe(
            ...
            column_config={
                ...
                "Username": st.column_config.TextColumn("👤 Actor"),
                "Action": st.column_config.TextColumn("🏷️ Action"),
                "Description": st.column_config.TextColumn("📝 Description"),
                "Request_ID": st.column_config.TextColumn("ID")
            }
        )
        ...
        col2.metric("Total Logged Events", len(log_df))
        st.caption("💡 Showing the latest 100 activities extracted from request history.")
    ...
    with tab3:
            ...
            Category = st.selectbox(
                "🔍 Select a Comparison Table to View:",[
                    ...
                    "Printing Parameters", 
                    "Price Comparison", 
                    "General Recommendations"], key="box1")
